	...
	with tf.Session(graph=detection_graph) as sess:
		...
		for image_path, result_path in zip(TEST_IMAGE_PATHS, RESULT_IMAGE_PATHS):
			...
			if os.path.exists(result_path) == False:
				...
				#testimg = np.zeros(image_np.shape)
				#cv2.imwrite(result_path, testimg)
			...
			#	line_thickness=1)
			#	print(result_path)
                vis_util.visualize_boxes_and_labels_on_image_array(image_np,image_path,
                ...
                cv2.imwrite(join(PATH_TO_RESULT_IMAGES_DIR, image_name), image_np)
